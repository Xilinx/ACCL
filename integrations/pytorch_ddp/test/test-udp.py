...
def start_test(simulator: bool, *,
    ...
    rank = mpi.Get_rank()
    size = mpi.Get_size()
    ranks = [accl.Rank("127.0.0.1" if simulator else f"10.10.10.{i}", 5500 + i,
                       i, rxbufsize) for i in range(size)]

    if simulator:
        accl.create_simulate_process_group(ranks, udp=True, bufsize=rxbufsize)
    else:
        accl.create_process_group(ranks, xclbin, device_index,
                                  accl.ACCLDesign.udp, bufsize=rxbufsize)
