...
def test_broadcast():
    if rank == 0:
        ...
        x = torch.zeros(count, device='cuda')

    dist.broadcast(x, 0)

    np.testing.assert_allclose(x.cpu(), torch.ones(count))
    print("Test broadcast finished!")


def test_sendrcv():
    x = torch.full((count,), float(rank), device='cuda')

    y = torch.empty(count, device='cuda')
    ...
    np.testing.assert_allclose(y.cpu(), torch.full((count,), prev_rank))
    print("Test sendrcv finished!")


def test_scatter():
    if rank == 0:
        x = [torch.full((count,), float(i), device='cuda') for i in range(size)]
    else:
        x = None
    y = torch.empty(count, device='cuda')
    ...
    np.testing.assert_allclose(y.cpu(), torch.full((count,), float(rank)))
...
def test_gather():
    ...
    if rank == 0:
        y = [torch.empty(count, device='cuda') for _ in range(size)]
    else:
        y = None
    ...
    if rank == 0:
        for i, c in enumerate(y):
            np.testing.assert_allclose(c.cpu(), torch.full((count,), float(i)))
...
def test_reduce():
    ...
    if rank == 0:
        np.testing.assert_allclose(x.cpu(), [size for _ in range(count)])
...
def start_test(simulator: bool, *,
    ...
    rank = mpi.Get_rank()
    size = mpi.Get_size()
    ranks = [accl.Rank("127.0.0.1", 5500 + i, i, 16 * 1024)
             for i in range(size)]

    if simulator:
        accl.create_simulate_process_group(ranks)
    else:
        accl.create_process_group(ranks, xclbin, device_index,
                                  accl.ACCLDesign.axis3x, p2p_enabled=True)
