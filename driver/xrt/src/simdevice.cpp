...
SimDevice::SimDevice(unsigned int zmqport, unsigned int local_rank) {
    debug("SimDevice connecting to ZMQ on port " + std::to_string(zmqport) + " for rank " + std::to_string(local_rank));
...
void SimDevice::start(const Options &options) {
    ...
    zmq_client_startcall(&(this->zmq_ctx), static_cast<int>(options.scenario), options.tag, 
                            options.count, options.comm, options.root_src_dst, function,
                            options.arithcfg_addr, 
                            static_cast<int>(options.compression_flags),
                            static_cast<int>(options.stream_flags),
                            options.addr_0->physical_address(),
                            options.addr_1->physical_address(),
