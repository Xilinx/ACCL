...
// DEBUG
// void CoyoteDevice::write_TLB_entry(addr_t vaddr, addr_t paddr, uint32_t len) {
//   this->coyote_proc->explicitTLBMap((void *) vaddr, (void *) paddr, len);
// }

/**
 * Attach the FPGA to a DMABuf. Setup the content of the TLBs accordingly.
 */
void CoyoteDevice::attach_dma_buf(uint64_t buf_fd, addr_t vaddr, uint32_t offset) {
  this->coyote_proc->attachDMABuf(buf_fd, (void *)vaddr, offset);
}
