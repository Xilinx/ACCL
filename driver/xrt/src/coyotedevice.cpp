...
void CoyoteDevice::start(const Options &options) {
  ...
  if (coyote_proc.getCSR(OFFSET_HOSTCTRL + HOSTCTRL_ADDR::AP_CTRL) & 0x02 == 0) { // read AP_CTRL and check bit 2 (the done bit)
  ...
  coyote_proc.setCSR(static_cast<uint32_t>(options.stream_flags), (OFFSET_HOSTCTRL + HOSTCTRL_ADDR::STREAM_FLAGS) >> 2);
  ...
  // start the kernel
  coyote_proc.setCSR(0x1U, OFFSET_HOSTCTRL + HOSTCTRL_ADDR::AP_CTRL);

  auto end = std::chrono::high_resolution_clock::now();
  durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);

  std::cout<<"Invocation latency:"<<durationUs<<"[us]"<<std::endl;
}
...
void CoyoteDevice::wait() {
  ...
  while (!is_done) {
    uint32_t regi = coyote_proc.getCSR(OFFSET_HOSTCTRL + HOSTCTRL_ADDR::AP_CTRL);
    if (last != regi) {
