...
void test_reduce_send(ACCL::ACCL &accl, options_t &options, int root,
                 reduceFunction function) {
  std::cout << "Start reduce-send test..." << std::endl;
  unsigned int count = options.count;
  auto op_buf = accl.create_buffer<float>(count, dataType::float32);
  auto res_buf = accl.create_buffer<float>(count, dataType::float32);
  random_array(op_buf->buffer(), count);
  ...
  test_debug("Reduce data to " + std::to_string(root) + "...", options);
  accl.reduce_send(dataType::float32, *res_buf, count, root, function);
...
int start_test(options_t options) {
  ...
  for (int root = 0; root < size; ++root) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    test_reduce_send(*accl, options, root, reduceFunction::SUM);
    MPI_Barrier(MPI_COMM_WORLD);
  }
