 ...
 */
void cProcess::attachDMABuf(uint64_t buf_fd, void * vaddr, uint32_t offset) {
	uint64_t tmp[4];
	tmp[0] = buf_fd;
	tmp[1] = reinterpret_cast<uint64_t>(vaddr);
	tmp[2] = offset;
	tmp[3] = static_cast<uint64_t>(cpid);
  	if(ioctl(fd, IOCTL_ATTACH_DMABUF, &tmp))
		throw std::runtime_error("p2p_attach_dma_buf() failed");

	DBG3("DMABuf attached.");
}
 ...
 */
void cProcess::detachDMABuf(uint64_t buf_fd) {
	uint64_t tmp[2];
	tmp[0] = buf_fd;
	tmp[1] = static_cast<uint64_t>(cpid);

	if(ioctl(fd, IOCTL_DETACH_DMABUF, &tmp))
		throw std::runtime_error("p2p_detach_dma_buf() failed");

	DBG3("DMABuf detached.");
}
 ...
 */
uint64_t cProcess::exportMemWithDMABuf(void * vaddr, uint32_t size, int * buf_fd) {
	uint64_t tmp[3];
	tmp[0] = reinterpret_cast<uint64_t>(vaddr);
 ...
 * @return the file descriptor of the DMABuf
 */
uint64_t cProcess::exportCTRL( int * buf_fd) {
...
*/
int cProcess::closeExportedDMABuf(uint64_t buf_fd) {
	uint64_t tmp[1];
	tmp[0] = buf_fd;
...
// */
// uint64_t cProcess::importDMABufFromFDToGPU(int buf_fd) {
// 	uint64_t tmp[2];
// 	tmp[0] = static_cast<uint64_t>(buf_fd);

// 	if(ioctl(fd, IOCTL_DMABUF_FROM_FD_TO_GPU, &tmp))
// 		throw std::runtime_error("dma_buf_from_fd_to_gpu() failed");

// 	uint64_t final_addr = reinterpret_cast<uint64_t>(tmp[1]);

// 	DBG3("Returned final addres: " << std::hex << final_addr << std::dec );
...
//  * @param buf_fd - the file descriptor of the DMABuf
// */
// void cProcess::detachDMABufFromFDToGPU(int buf_fd) {
// 	uint64_t tmp[2];
// 	tmp[0] = static_cast<uint64_t>(buf_fd);
// 	tmp[1] = static_cast<uint64_t>(cpid);

// 	DBG3("buf_fd = " << tmp[0] << ", cpid: " << cpid << std::endl);

// 	if(ioctl(fd, IOCTL_DMABUF_DETACH_FROM_FD_TO_GPU, &tmp))
// 		throw std::runtime_error("dma_buf_export_regs() failed");

// 	DBG3("DMABuf for FPGA memory detached.");
...
//  * @param len - length (number of pages)
//  */
// void cProcess::explicitTLBMap(void *vaddr, void *paddr, uint32_t len) {
// 	uint64_t tmp[4];
// 	tmp[0] = reinterpret_cast<uint64_t>(vaddr);
// 	tmp[1] = reinterpret_cast<uint64_t>(paddr);
// 	tmp[2] = static_cast<uint64_t>(len);
// 	tmp[3] = static_cast<uint64_t>(cpid);

// 	if(ioctl(fd, IOCTL_MAP_EXPLICIT, &tmp))
// 		throw std::runtime_error("ioctl_map_user() failed");

// 	mapped_upages.emplace(vaddr); //what to do here??
// 	DBG3("Explicit map user mem at: " << std::hex << reinterpret_cast<uint64_t>(vaddr) << std::dec);
// }
...
//  * @param paddr - physical address
//  */
// void cProcess::explicitTLBUnmap(void *vaddr) {
// 	uint64_t tmp[2];
// 	tmp[0] = reinterpret_cast<uint64_t>(vaddr);
// 	tmp[1] = static_cast<uint64_t>(cpid);

// 	if(mapped_upages.find(vaddr) != mapped_upages.end()) {
// 		if(ioctl(fd, IOCTL_UNMAP_EXPLICIT, &tmp)) 
// 			throw std::runtime_error("ioctl_unmap_user() failed");

// 	//	mapped_upages.erase(vaddr); //already commented out, do not touch
// 	}	
// }
